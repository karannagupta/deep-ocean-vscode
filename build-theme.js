...
const malformed = Object.entries(colors).filter(
  ([, v]) => !/^#([0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i.test(v)
);
if (malformed.length) {
...
fs.writeFileSync(
  ...
  JSON.stringify(
    { name: 'Deep Ocean', type: 'dark', include: './base-dark_vs.json', colors },
    null,
    2
  ) + '\n'
);
...
console.log(
  `themes/deep-ocean-color-theme.json — ${Object.keys(colors).length} colors ` +
    `(${dropped} left to the base theme).`
);
